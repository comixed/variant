...
open class VariantViewModel(val settings: Settings, val directoryRepository: DirectoryRepository) :
  ViewModel() {
  ...
  @NativeCoroutinesState var comicBook: StateFlow<ComicBook?> = _comicBook.asStateFlow()

  private val _browsingState =
    MutableStateFlow<BrowsingState>(
      viewModelScope,
      BrowsingState(
  ...
  @NativeCoroutinesState val browsingState: StateFlow<BrowsingState> = _browsingState.asStateFlow()

  private val _loading = MutableStateFlow<Boolean>(viewModelScope, false)

  @NativeCoroutinesState val loading: StateFlow<Boolean> = _loading.asStateFlow()

  private val _comicBookList = MutableStateFlow<List<ComicBook>>(viewModelScope, listOf())
  ...
  @NativeCoroutinesState val selectionMode: StateFlow<Boolean> = _selectionMode.asStateFlow()

  private val _selectionList = MutableStateFlow<List<String>>(viewModelScope, listOf())

  @NativeCoroutinesState val selectionList: StateFlow<List<String>> = _selectionList.asStateFlow()
    ...
    viewModelScope.launch(Dispatchers.Main) {
      ...
      if (contents.isEmpty() || reload) {
        ...
        try {
          ...
          Log.debug(TAG, "Loading directory contents: url=${url} reload=${reload}")
          contents =
            ReaderAPI.loadDirectory(url, username, password).contents.map {
              DirectoryEntry(
                id = null,
  ...
  fun downloadFile(path: String, filename: String) {
    ...
    viewModelScope.launch(Dispatchers.IO) {
        ...
        ReaderAPI.downloadComic(
          ...
          onProgress = { received, total ->
            viewModelScope.launch(Dispatchers.Main) {
              ...
              val state =
                _browsingState.value.downloadingState.filter { !(it.path == path) }.toMutableList()
              state.add(downloadingState)
              Log.debug(
                TAG,
      ...
      val finalState =
        _browsingState.value.downloadingState.filter { !(it.path.equals(path)) }.toMutableList()
  ...
  private suspend fun onLibraryFileEvent(file: File, event: KfsEvent) {
    ...
    when (event) {
      ...
      KfsEvent.Delete -> {
        val contents = _comicBookList.value.filter { !it.path.equals(file.fullPath) }
