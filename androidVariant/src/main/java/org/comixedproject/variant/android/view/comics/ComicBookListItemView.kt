...
fun ComicBookListItemView(
  ...
  ElevatedCard(
    ...
    modifier = modifier.fillMaxWidth().border(borderWidth, Color.Red),
  ) {
    val title = MetadataAPI.displayableTitle(comicBook)

    Column(modifier = Modifier.combinedClickable(onClick = { onClick(comicBook) })) {
      comicBook.pages.firstOrNull()?.let { cover ->
        if (coverContent == null) {
          ...
          coroutineScope.launch(Dispatchers.Main) {
            Log.debug(TAG, "Loading content for ${comicBook.filename}:${cover.filename}")
            coverContent = ArchiveAPI.loadCover(comicBook.path, cover.filename)
          }
        } else {
...
fun ComicBookListItemViewPreview() {
  VariantTheme { ComicBookListItemView(comicBook = COMIC_BOOK_LIST.get(0), false, onClick = {}) }
}

@Composable
@Preview
fun ComicBookListItemViewSelectedPreview() {
  VariantTheme { ComicBookListItemView(comicBook = COMIC_BOOK_LIST.get(0), true, onClick = {}) }
}
